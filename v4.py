...
import requests, tkinter, threading, webbrowser, random, datetime, time, json
from tkinter import *
...
def getDomainName(url):
    ...
    url=domain.replace("http://","").replace("https://","").replace("/","")
    return url
...
def getContactEmail():
    if(tree.focus()==""):
        ...
        url=links[int(tree.focus()[1:], 16)-1-index_so_far]
        parsed_uri = urlparse(url)
        domain = '{uri.scheme}://{uri.netloc}/'.format(uri=parsed_uri)
        ##
        url=domain.replace("http://","").replace("https://","").replace("/","")
        print(url)
        try:
            json_data=getBsObj("http://api.bulkwhoisapi.com/whoisAPI.php?domain="+url+"&token=usemeforfree")
            if("failed" in json_data.get_text()):
                tkinter.messagebox.showerror("Error","The API is unavailable at this time. Please try again")
                print(json_data.get_text())
            else:
                loaded_json = json.loads(json_data.get_text())
                email=loaded_json["formatted_data"]["RegistrarAbuseContactEmail"]
                print(email)
                tkinter.messagebox.showinfo("Success","Email sent to "+email)
...
class mainThread (threading.Thread):
    ...
    ##                    try:
                    bsObj2=getBsObj(x)
                    all_link_objects=bsObj2.findAll('a')
                    all_links=[]
                    for x in all_link_objects:
                        try:
    ...
    ##                        status.set("Status: Searching...")
                    ...
                    for y in sites:
                        for x in all_links:
                            if(y.lower() in getDomainName(x).lower()):
                                #print('infected')
                                #t=title
                                #x=link
...
def main():
    if(status.get()=="Status: Idle"):
        if(level.get()==0 or product.get()==""):
            tkinter.messagebox.showerror("Error","Please re-check your level and product!")
        else:
            thread1=mainThread()
            thread1.start()
    else:
...
bg_img=PhotoImage(file="D:\\tiled.png")
background_label = Label(f1, image=bg_img)
#Space Fillers:
